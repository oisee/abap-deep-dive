    ...
    subgraph "Request Flow Architecture"
        ...
        subgraph "Application Server"
            subgraph "Dispatcher Process"
                LISTENER[Network Listener<br/>Port 32XX]
...
1. **Прием соединений**: Слушает на выделенном порту (обычно 32XX, где XX - номер инстанции)
...
```c
// Псевдо-структура элемента очереди диспетчера
typedef struct {
    uint32_t request_id;      // Уникальный ID запроса
    uint16_t request_type;    // DIA, BTC, UPD, etc.
    uint32_t user_context;    // Указатель на контекст пользователя
    uint16_t priority;        // Приоритет обработки
    timestamp_t arrival_time; // Время поступления
    uint32_t client;         // Клиент системы
    char user[12];           // Имя пользователя
    char tcode[20];          // Транзакция
    void* request_data;      // Данные запроса
} dispatcher_queue_entry_t;
```
...
DIAG (Dynamic Information and Action Gateway) — это проприетарный протокол SAP для коммуникации между GUI и Application Server. Ключевые особенности:

- **Компрессия**: Передаются только изменения экрана, не весь экран
- **Stateful**: Поддерживает состояние сессии
- **Оптимизирован**: Минимальный сетевой трафик

Пример DIAG пакета (упрощенно):
...
Value: /nSE80
Fields_Changed: 1
Field[0]: { ID: "PROGRAM", Value: "ZTEST" }
```
